...
import MyCalendar8 from './calendars/MyCalendar8'; // 匯入自定義的 MyCalendar8 日曆組件
...
import MyModal from './utils/Modal'; // 匯入自定義的 Modal 組件

// 定義 Calendar8 功能組件
const Calendar8 = () => {

  // 狀態管理，用於控制 Modal 的開啟和顯示選擇的日期
  const [open, setOpen] = useState(false); // 控制 Modal 的開關
  const [selectedDate, setSelectedDate] = useState([false]); // 保存選擇的日期
